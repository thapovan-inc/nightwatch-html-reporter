...
var now = new Date();
var timeStamp = moment().tz("America/New_York").format("MM/DD/YYYY HH:mm:ss A");

var country = process.env.COUNTRYNAME;
var environment = process.env.TEST_ENV;
var summary = process.env.SUMMARY;
    ...
    _.each(results, function(result) {
      ...
      _.each(result.testsuites.testsuite, function(suiteData) {
        ...
        _.each(suiteData.testcase, function(caseData) {
          ...
          var sspath = [`${options.screenshotsDirectory}/${assert.message}.png`];
          assert.screenshots.push(sspath);
    ...
    _.forOwn(results.modules, function(pkg, pkgName) {
      ...
      _.forOwn(pkg.completed, function(suite, suiteName) {
        ...
        _.each(suite.assertions, function(assertion) {
          ...
          var screenshot = `${options.screenshotsDirectory}/${assert.message}.png`;
          assert.screenshots = assert.screenshots || [];
          if(fs.existsSync(screenshot)) {
            assert.screenshots.push(screenshot);
          }

          assert.screenshots.forEach(function(value, index) {
            var encodedValue = value.split('/').map(function (entry) { return encodeURIComponent(entry) }).join('/');
            assert.screenshots[index] = encodedValue;
          });
